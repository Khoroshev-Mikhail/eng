...
import { ReferencesGroup } from "../References/ReferencesGroup";

export default function GroupPage(){
    const { id_group } = useParams()
    const dispatch = useAppDispatch()
    const vocabulary = useAppSelector(getVocabulary)
    const { group: { words }, status} = useAppSelector(getGroup)
    const progress = getGroupProgress(vocabulary, words)
    useEffect(()=>{

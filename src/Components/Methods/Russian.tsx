...
import { getVocabularyRussian, setVocabularyAndGetUpdatedVocabularyThunk } from '../../app/clientAPI/vocabularySliceAPI'
...
export default function Russian(){
    ...
    const vocabularyRussian = useAppSelector(getVocabularyRussian)
    ...
    const answer = (word_id: number) => {
        if(trueVariant.id === word_id){
            dispatch(setVocabularyAndGetUpdatedVocabularyThunk({method: 'russian', word_id }))
        } else{
            if(user.id && id_group){
    ...
    }, [id_group, user.id, vocabularyRussian])
    return(
        <>  

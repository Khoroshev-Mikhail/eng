...
export default function English(){
    ...
    function setAnswers(){
        setWord(unlernedWord(vocabulary.english, wordsByGroup))
        word && setFalses(falseVariants(wordsByGroup, word))
    }
    useEffect(()=>{
        ...
        <Card centered fluid={document.documentElement.clientWidth < 800 ? true : false}>
            ...
            <Card.Content>
                {falses && falses.map((el: WordType, i: number) => {
                    return <Button onClick={el.eng === word?.eng ? ()=>setVocabulary({word_id: el.id, user_id: 1, method: 'english'}) : ()=>{setAnswers()}} key={id+i} fluid basic color='green' size='huge' className={css.marginButtons}>{el ? el.rus : ' '}</Button>
                })}
            </Card.Content>
        </Card>

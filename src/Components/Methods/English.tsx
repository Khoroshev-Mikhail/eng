...
export default function English(){
    ...
    const { data, isSuccess, isError, refetch } = useGetUnlernedQuery({ id_user: user.id || 0, method: 'english', id_group }) //костыль
    ...
    const answer = (word_id: number) => {
        if(isSuccess && data !== null && data.trueVariant.id === word_id ){
            setVocabulary({ id_user: user.id || 0, method: 'english', word_id })
        } else{
            if(user.id && id_group){
                refetch()
            }
